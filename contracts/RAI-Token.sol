...
contract ERC20Token is ERC20Permit, AccessControl {
    ...
    modifier onlyMinter() {
        require(hasRole(MINT, msg.sender), "NotMinter: caller does not have MINTER role");
        _;
    }
    ...
    function _burnFrom(address account_, uint256 amount_) public virtual {
        ...
        uint256 decreasedAllowance_ = currentAllowance - amount_;
...
contract RAIToken is ERC20Token {
    ...
    address public feeReceiver = 0x3EE90695ADbfD84bEdf710Aab2a17E718B311235;// DAO contract
    string public constant nameToken ="RAI";
    string public constant symbolToken="RAI";

    uint256 public constant PRECISION = 100e3;
    uint256 MAX_ALLOWED_FEE_RATIO = 20e3;
    ...
    event FeeReceiverRoleGranted(address indexed feeReceiver, bytes32 role); 
    ...
    function setMainPair(address pair) external onlyDefaultAdmin {
        require(pair != address(0), "Contract address cannot be zero");
        mainPair = pair;
        _setupRole(COOLING_PROTECTOR, mainPair);
    }
    function setFeeReceiver(address _feeReceiver) external onlyDefaultAdmin {
        require(_feeReceiver != address(0), "Contract address cannot be zero");
        feeReceiver = _feeReceiver;
        _setupRole(INTERN_SYSTEM, feeReceiver);
        emit FeeReceiverRoleGranted(_feeReceiver, INTERN_SYSTEM);
        _setupRole(COOLING_PROTECTOR, feeReceiver);
        emit FeeReceiverRoleGranted(_feeReceiver, COOLING_PROTECTOR); 
    }
   function setMintRole(address account) external onlyDefaultAdmin returns (bytes32) {
        require(account != address(0), "Contract address cannot be zero");
    ...
    function setRatio(uint8 ratioType,uint256 ratio) external onlyDefaultAdmin {
        ...
        require(ratio <= MAX_ALLOWED_FEE_RATIO, "Fee ratio exceeds max allowed");
        if(ratioType ==0){
            buyFeeRatio = ratio;
        } else {
            sellFeeRatio = ratio;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (_isTradeAndNotInSystem(sender, recipient)) {
            // buyer or remove lp
            if (sender == mainPair) {
                uint buyFee = (amount * buyFeeRatio) / PRECISION;
                if(buyFee>0){
                    amount = amount - buyFee;
                    _balances[feeReceiver] = _balances[feeReceiver] + buyFee;
                ...
                // take fee
                uint256 sellFee = (amount * sellFeeRatio) / PRECISION;
                if (sellFee > 0) {
                    amount = amount - sellFee;
                    _balances[feeReceiver] = _balances[feeReceiver] + sellFee;
